export class HSalsa20 {
    ...
    rotate(u: number, c: number): number {
        return (u << c) | (u >>> (32 - c));
    }

    load_littleendian(x: Buffer, offset: number): number {
    ...
    store_littleendian(x: Buffer, offset: number, u: number): void {
        x[offset] = this.toByte(u);
        u >>>= 8;
        x[offset + 1] = this.toByte(u);
        u >>>= 8;
        x[offset + 2] = this.toByte(u);
        u >>>= 8;
        x[offset + 3] = this.toByte(u);
    }

    crypto_core(outv: Buffer, inv: Buffer, k: Buffer, c: Buffer): void {
        let x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15;
        let j6, j7, j8, j9;
        let i;

        const j0 = x0 = this.load_littleendian(c, 0);
        const j1 = x1 = this.load_littleendian(k, 0);
        const j2 = x2 = this.load_littleendian(k, 4);
        const j3 = x3 = this.load_littleendian(k, 8);
        const j4 = x4 = this.load_littleendian(k, 12);
        const j5 = x5 = this.load_littleendian(c, 4);
        ...
        const j10 = x10 = this.load_littleendian(c, 8);
        const j11 = x11 = this.load_littleendian(k, 16);
        const j12 = x12 = this.load_littleendian(k, 20);
        const j13 = x13 = this.load_littleendian(k, 24);
        const j14 = x14 = this.load_littleendian(k, 28);
        const j15 = x15 = this.load_littleendian(c, 12);
